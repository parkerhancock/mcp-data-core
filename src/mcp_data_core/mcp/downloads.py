...
def _cache_dir() -> Path:
    ...
    if custom:
        return Path(custom)
    return Path.home() / ".cache" / "mcp_data_core" / "downloads"
...
async def build_download_url_or_fetch(
    ...
    *,
    ...
    tmp = tempfile.NamedTemporaryFile(suffix=suffix, delete=False, prefix="mcp_data_core_")
    tmp.write(content)
    tmp.close()
...
async def download_bulk_response(
    ...
    *,
    ...
    into a tempfile under ``~/.cache/mcp_data_core/bulk_zips/{uuid}.zip``,
